...
const Chat = () => {
  ...
  const lobbyname = new URLSearchParams(window.location.search).get('lobbyname')
    ...
    <div className="chat-main">
      ...
      <div className="row chat-message">
        <div className="chat-sidebar col-lg-3 ">
          <h2>{username}</h2>
          <Button className="chat-btn1" onClick={() => leaveLobby()} >Leave Lobby</Button>
          <Button className="chat-btn2" onClick={() => signOut()}>Sign out</Button>
          <h3>Lobby Name</h3>
          <h4>{lobbyname}</h4>
          <h3>People</h3>
          <div>
        ...
        ? messages.map((message, index) => (
            <div
              className={`msg-container${
                message.username === username ? '-right' : message.username !== 'ChatCord Bot' ? '-left' : ''
              }`}
              key={index}
            >
            {message.username !== 'ChatCord Bot' &&
              ...
              <div className="msg-content"> {message.text}
              </div>
